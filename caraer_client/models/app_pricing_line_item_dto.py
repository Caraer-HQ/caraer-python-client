...
class AppPricingLineItemDTO(BaseModel):
    """
    ...
    count_type: Optional[StrictStr] = Field(default=None, description="Count type: meter or static_query", alias="countType")
    counting_source: Optional[StrictStr] = Field(default=None, description="Event sources for meter line items: WEBHOOK, MANUAL, WEBHOOK_AND_MANUAL", alias="countingSource")
    unit: Optional[StrictStr] = Field(default=None, description="Unit label (e.g. webhook, document)")
    included_units: Optional[StrictStr] = Field(default=None, description="Included units for FLAT line items", alias="includedUnits")
    price_per_unit: Optional[StrictStr] = Field(default=None, description="Base price covering included units (FLAT)", alias="pricePerUnit")
    price_per_extra_unit: Optional[StrictStr] = Field(default=None, description="Price per unit above includedUnits (FLAT)", alias="pricePerExtraUnit")
    static_query: Optional[AppStaticQueryDTO] = Field(default=None, description="Scheduled Cypher snapshot for static_query line items", alias="staticQuery")
    tiers: Optional[List[AppTierDTO]] = Field(default=None, description="Tiers for TIERED line items")
    __properties: ClassVar[List[str]] = ["uuid", "name", "label", "createdAt", "createdBy", "updatedAt", "updatedBy", "deletedAt", "deletedBy", "index", "countType", "countingSource", "unit", "includedUnits", "pricePerUnit", "pricePerExtraUnit", "staticQuery", "tiers"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of static_query
        if self.static_query:
            _dict['staticQuery'] = self.static_query.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "pricePerExtraUnit": obj.get("pricePerExtraUnit"),
            "staticQuery": AppStaticQueryDTO.from_dict(obj["staticQuery"]) if obj.get("staticQuery") is not None else None,
            "tiers": [AppTierDTO.from_dict(_item) for _item in obj["tiers"]] if obj.get("tiers") is not None else None
        })
        return _obj
