...
class AppStaticQueryDTO(BaseModel):
    """
    Read-only Cypher snapshot used by static_query line items
    """ # noqa: E501
    schedule: Optional[StrictStr] = Field(default=None, description="When the query runs: period_end (default) or period_start")
    cypher: Optional[StrictStr] = Field(default=None, description="Read-only Cypher that returns count(...) AS count")
    __properties: ClassVar[List[str]] = ["schedule", "cypher"]
