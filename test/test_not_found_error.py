# coding: utf-8

"""
    Caraer API Documentation

    This documentation provides an overview of the Caraer API, including authentication methods, endpoint details, data models, and integration guidelines for building on the Caraer platform.

    The version of the OpenAPI document: 2.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from caraer_client.models.not_found_error import NotFoundError

class TestNotFoundError(unittest.TestCase):
    """NotFoundError unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> NotFoundError:
        """Test NotFoundError
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `NotFoundError`
        """
        model = NotFoundError()
        if include_optional:
            return NotFoundError(
                cause = caraer_client.models.not_found_error_cause.NotFoundError_cause(
                    stack_trace = [
                        caraer_client.models.not_found_error_cause_stack_trace_inner.NotFoundError_cause_stackTrace_inner(
                            class_loader_name = '', 
                            module_name = '', 
                            module_version = '', 
                            method_name = '', 
                            file_name = '', 
                            line_number = 56, 
                            class_name = '', 
                            native_method = True, )
                        ], 
                    message = '', 
                    localized_message = '', ),
                stack_trace = [
                    caraer_client.models.not_found_error_cause_stack_trace_inner.NotFoundError_cause_stackTrace_inner(
                        class_loader_name = '', 
                        module_name = '', 
                        module_version = '', 
                        method_name = '', 
                        file_name = '', 
                        line_number = 56, 
                        class_name = '', 
                        native_method = True, )
                    ],
                errors = Resource not found.,
                code = 404,
                correction_suggestion = 'Try uploading a different file type.',
                message = 'Invalid file type.',
                suppressed = [
                    caraer_client.models.not_found_error_cause.NotFoundError_cause(
                        stack_trace = [
                            caraer_client.models.not_found_error_cause_stack_trace_inner.NotFoundError_cause_stackTrace_inner(
                                class_loader_name = '', 
                                module_name = '', 
                                module_version = '', 
                                method_name = '', 
                                file_name = '', 
                                line_number = 56, 
                                class_name = '', 
                                native_method = True, )
                            ], 
                        message = '', 
                        localized_message = '', )
                    ],
                localized_message = '',
                resource_kind = '',
                identifier = '',
                identifier_type = '',
                referenced_by = [
                    caraer_client.models.reference.Reference(
                        kind = '', 
                        identifier = '', 
                        label = '', 
                        field = '', )
                    ]
            )
        else:
            return NotFoundError(
                correction_suggestion = 'Try uploading a different file type.',
                message = 'Invalid file type.',
        )
        """

    def testNotFoundError(self):
        """Test NotFoundError"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
