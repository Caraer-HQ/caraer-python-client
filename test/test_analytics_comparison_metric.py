# coding: utf-8

"""
    Caraer API Documentation

    This documentation provides an overview of the Caraer API, including authentication methods, endpoint details, data models, and integration guidelines for building on the Caraer platform.

    The version of the OpenAPI document: 2.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from caraer_client.models.analytics_comparison_metric import AnalyticsComparisonMetric

class TestAnalyticsComparisonMetric(unittest.TestCase):
    """AnalyticsComparisonMetric unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AnalyticsComparisonMetric:
        """Test AnalyticsComparisonMetric
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AnalyticsComparisonMetric`
        """
        model = AnalyticsComparisonMetric()
        if include_optional:
            return AnalyticsComparisonMetric(
                key = '',
                label = '',
                main_object = '',
                filter = caraer_client.models.filter.Filter(
                    groups = [
                        caraer_client.models.filter_group.FilterGroup(
                            items = null, )
                        ], ),
                y_axis = caraer_client.models.analytics_axis_config.AnalyticsAxisConfig(
                    property = caraer_client.models.analytics_property_ref.AnalyticsPropertyRef(
                        object = '', 
                        relation = '', 
                        property_name = '', ), 
                    time_bucket = '', 
                    time_zone = '', 
                    window_days = 56, 
                    bin_count = 56, 
                    metric = '', 
                    label = '', )
            )
        else:
            return AnalyticsComparisonMetric(
        )
        """

    def testAnalyticsComparisonMetric(self):
        """Test AnalyticsComparisonMetric"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
