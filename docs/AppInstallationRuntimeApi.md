...
> ShowResponseMapStringObject trigger_setting_action(app_uuid, field_name)
...
with caraer_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Trigger an ACTION setting without saving settings
        api_response = api_instance.trigger_setting_action(app_uuid, field_name)
        print("The response of AppInstallationRuntimeApi->trigger_setting_action:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AppInstallationRuntimeApi->trigger_setting_action: %s\n" % e)
```
